...
export function FollowUpsPage({ onStartCalling }: { onStartCalling: () => void }) {
  ...
  return <>
    ...
    <section className="panel followups-panel">
      ...
      })}</div> : <EmptyState icon={<Icon name={tab === "dormant" ? "history" : "followUp"} size={28} />} title={tab === "due" ? "No follow-ups due today" : tab === "interested" ? "No interested prospects waiting" : `No ${tab.replace("_", " ")} follow-ups`} description="Post-meeting opportunities move here automatically and remain visible through all five touches." />}
    </section>
