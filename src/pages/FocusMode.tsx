        ...
        <section className="focus-lead">
          ...
          {isPostMeeting ? (
            ...
            <div className="finding-card">
              <div className="finding-card__top"><span className="finding-card__icon"><Icon name="shield" size={20} /></span><div><small>{lead.primaryFinding ? "Primary finding" : "Tracking Technology Found"}</small><h2>{lead.primaryFinding || lead.trackingTechnologyFound || "Unknown"}</h2></div>{lead.primaryFinding && lead.findingStrength !== "unknown" ? <span className={cn("finding-strength", `is-${lead.findingStrength.toLowerCase()}`)}><strong>{lead.findingStrength}</strong><small>{lead.findingStrength === "A" ? "Strong" : lead.findingStrength === "B" ? "Good" : "Fallback"}</small></span> : null}</div>
              {lead.evidenceNotes || lead.pitchNotes ? <div className="finding-card__detail">{lead.evidenceNotes ? <p><small>Evidence</small>{lead.evidenceNotes}</p> : null}{lead.pitchNotes ? <p><small>Pitch angle</small>{lead.pitchNotes}</p> : null}</div> : null}
              <div className="finding-card__tags">{lead.trackingTechnologyFound ? <Badge tone="purple">{lead.trackingTechnologyFound}</Badge> : null}{lead.securityGrade ? <Badge tone="warning">Grade {lead.securityGrade}</Badge> : null}{lead.websiteUrl ? <a href={/^https?:\/\//i.test(lead.websiteUrl) ? lead.websiteUrl : `https://${lead.websiteUrl}`} target="_blank" rel="noreferrer"><Icon name="externalLink" size={14} /> Open website</a> : null}</div>
            </div>
          )}
